...
class MainWindow:
    ...
    def create_ai_configuration_section(self, parent):
        ...
        attach_frame = tk.Frame(content, bg=Colors.SURFACE)
        attach_frame.pack(fill=tk.X, pady=Padding.SMALL)
        
        self.attachment_path = tk.StringVar()
        tk.Label(attach_frame, text="Attachment (Optional):", font=Fonts.NORMAL_BOLD, bg=Colors.SURFACE).pack(side=tk.LEFT)
        tk.Entry(attach_frame, textvariable=self.attachment_path, state='readonly', width=30).pack(side=tk.LEFT, padx=Padding.MEDIUM)
        
        tk.Button(attach_frame, text="Select File", command=self._browse_attachment, font=Fonts.SMALL).pack(side=tk.LEFT)
    ...
    def _browse_attachment(self):
        ...
        if file_path:
            ...
            self.log_status(f"Attachment selected: {os.path.basename(file_path)}")
    ...
    def _send_messages_thread(self):
        ...
        try:
            ...
            # Get the attachment path from the GUI variable
            current_attachment = self.attachment_path.get() if hasattr(self, 'attachment_path') else None
            ...
            #report_path = self._generate_delivery_report(stats)
